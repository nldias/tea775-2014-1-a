...
$Q(x) = \left\{ x \boxplus y, \; y \in Q \right\} = Q\blob$ (Ailin, esta dedução
    é \emph{diferente} de suas notas.  Estou tentando tirar o máximo partido de
    `$\boxplus$'!  Está certo?)
\item
\begin{align}
...
\]
(note a disjunção). Na sequência, precisamos provar (\ref{eq:irraQx1x2})--(\ref{eq:raciQx1x2}) e
prosseguir na obtenção do conjunto $T$. Esse último na verdade se revelará um
conjunto interessante, e na verdade o ponto final desta lição: $T$ se revelará
um conjunto \emph{não mensurável}.
...
onde $r$ é racional, e não necessariamente está em $Q$. Substitua:
\[
...
necessariamente $ p \in Q$).  Portanto, é possível escrever o lado direito
como $x_2 \boxplus z$, onde $z \in Q$.  Disso se segue que 
\[
...
\end{align*}
Logo, existe algum $q \in Q$ tal que $t = x + q$; pelo mesmo motivo agora deve
...
0 = (t_i - t_j) + q
\]
onde $q$ é racional. Logo, $t_i - t_j$ é racional, donde $Q(t_i) = Q(t_j)$.  Mas
...
\]
Isso, nós também já vimos em outra lição, é impossível. $T$ não pode ser mensurável.
